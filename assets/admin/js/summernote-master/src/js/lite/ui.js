...
const button = renderer.create(
    '<button type="button" class="note-btn" role="button" tabindex="-1">', function ($node, options) {
        // set button type
...
const dropdown = renderer.create(
    '<div class="note-dropdown-menu" role="list">', function ($node, options) {
        const markup = $.isArray(options.items) ? options.items.map(
            function (item) {
        ...
        $node.on(
            'click', '> .note-dropdown-item', function (e) {
                const $a = $(this);
...
const dropdownCheck = renderer.create(
    '<div class="note-dropdown-menu note-check" role="list">', function ($node, options) {
        const markup = $.isArray(options.items) ? options.items.map(
            function (item) {
                const value = (typeof item === 'string') ? item : (item.value || '');
                const content = options.template ? options.template(item) : item;
        ...
        dropdown(
            {
                ...
                callback: function ($dropdown) {
                    $dropdown.find('.note-holder').each(
                        function () {
                            const $holder = $(this);
                            $holder.append(
                                palette(
...
const dialog = renderer.create(
    '<div class="note-modal" aria-hidden="false" tabindex="-1" role="dialog"/>', function ($node, options) {
        if (options.fade) {
            $node.addClass('fade');
        }
        $node.attr(
