 ...
 * @param integer $newwidth  Thumbnail width
 * @param integer $newheight Optional thumbnail height
...
}//end check_files_extensions_on_path()
...
}//end check_files_extensions_on_phar()
...
function config_loading($current_path, $fld)
{
    if (file_exists($current_path.$fld.'.config')) {
        include_once $current_path.$fld.'.config';
 ...
 *
 * @param string  $img
 * @param integer $max_breedte
 * @param integer $max_hoogte
 *
 * @return boolean
 */
...
function is_really_writable($dir)
{
    ...
    if (is_dir($dir)) {
        ...
        return true;
    } else if (! is_file($dir) || ($fp = @fopen($dir, 'ab')) === false) {
        return false;
    }
