...
class FtpClient implements Countable
{
     ...
     * @param  integer     $time_limit        The max execution time, unlimited by default
     ...
     * @param  boolean $recursive Forces deletion if the directory is not empty
     * @return boolean
     ...
     * @param  boolean $recursive Is effective only if $path is a directory, {@see FtpClient::rmdir()}
     * @return boolean
     */
    ...
    public function rawToType($permission)
    {
        if (!is_string($permission)) {
            throw new FtpException(
                'The "$permission" argument must be a string, "'.gettype($permission).'" given.'
            );
        }
