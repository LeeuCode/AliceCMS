...
function baseUrl($atRoot = false, $atCore = false, $parse = false)
{
    if (isset($_SERVER['HTTP_HOST'])) {
        ...
        $cores = preg_split('@/@', str_replace($_SERVER['DOCUMENT_ROOT'], '', realpath(dirname(__FILE__))), null, PREG_SPLIT_NO_EMPTY);
        $core  = $cores[0];
    ...
    // parseUrl($parse);
    return str_replace(['admin/', 'install/', '/filemanager/'], '', $baseUrl);
}//end baseUrl()
...
// only create files with exts. defined in $editable_text_file_exts
    ...
    'previewable_text_file_exts_no_prettify'  => [
        'txt',
        'log',
    ],
